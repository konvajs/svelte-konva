...
test('throws an error if not placed inside a Container (Layer, Group, Label) component', () => {
	expect(() => {
		render(Ring, {
			props: { x: 0, y: 0, innerRadius: 20, outerRadius: 100 }
		});
...
test('Can listen to Konva events', () => {
	...
	const rendered = render(Ring, {
		...
		props: {
			...{ x: 0, y: 0, innerRadius: 20, outerRadius: 100 },
			onmousedown: mockFn
		}
	});
