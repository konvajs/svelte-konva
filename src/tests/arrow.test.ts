...
test('throws an error if not placed inside a Container (Layer, Group, Label) component', () => {
	expect(() => {
		render(Arrow, {
			props: { x: 0, y: 0, points: [0, 0, 100, 100], strokeWidth: 10, stroke: 'black' }
		});
...
test('Can listen to Konva events', () => {
	...
	const rendered = render(Arrow, {
		...
		props: {
			...{ x: 0, y: 0, points: [0, 0, 100, 100], strokeWidth: 10, stroke: 'black' },
			onmousedown: mockFn
		}
	});
