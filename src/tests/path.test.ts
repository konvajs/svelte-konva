...
test('throws an error if not placed inside a Container (Layer, Group, Label) component', () => {
	expect(() => {
		render(Path, {
			props: { x: 0, y: 0, data: 'M 2 2 H 100 V 60 H 2 V 2 Z' }
		});
...
test('Can listen to Konva events', () => {
	...
	const rendered = render(Path, {
		...
		props: {
			...{ x: 0, y: 0, data: 'M 2 2 H 100 V 60 H 2 V 2 Z' },
			onmousedown: mockFn
		}
	});
