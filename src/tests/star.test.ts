...
test('throws an error if not placed inside a Container (Layer, Group, Label) component', () => {
	expect(() => {
		render(Star, {
			props: { x: 0, y: 0, innerRadius: 100, outerRadius: 200, numPoints: 5 }
		});
...
test('Can listen to Konva events', () => {
	...
	const rendered = render(Star, {
		...
		props: {
			...{ x: 0, y: 0, innerRadius: 100, outerRadius: 200, numPoints: 5 },
			onmousedown: mockFn
		}
	});
...
test('Correctly updates bound config on dragend', () => {
	const rawConfig = { x: 0, y: 0, innerRadius: 100, outerRadius: 200, numPoints: 5 };
