...
test('throws an error if not placed inside a Container (Layer, Group, Label) component', () => {
	expect(() => {
		render(Ellipse, {
			props: {
				config: { x: 0, radiusX: 120, radiusY: 70 }
			}
		});
...
test('Can listen to Konva events', () => {
	...
	const component = rendered.component.$$;
	const handle: Konva.Ellipse = component.ctx[component.props['handle'] as number];
...
test('Correctly updates bound config on dragend', () => {
	const rawConfig = { x: 0, radiusX: 120, radiusY: 70 };
